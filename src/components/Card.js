...
const Card = (props) => {
    ...
    const course = async () => {
        const data = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=${props.listId}&key=AIzaSyDbYK0Ag14Hq2En0PhG32o4ksktsuEgBLk`)
        const json = await data.json();
        setData(json.items);
    }
    //console.log(data)
    const playlistname = async () => {
        const res = await fetch(`https://www.googleapis.com/youtube/v3/playlists?id=${props.listId}&part=snippet&key=AIzaSyDbYK0Ag14Hq2En0PhG32o4ksktsuEgBLk`)
        ...
        <Fragment>
            <div className='grid grid-cols-2 gap-3'>
                <div className='flex flex-col gap-4 h-[30rem] px-10 overflow-scroll overflow-x-hidden'>
                    ...
                    {data && data?.map((item) => {
                        return <div className='flex cursor-pointer gap-1 hover:bg-gray-400 duration-100 p-2' style={{ border: "1px solid gray" }} onClick={() => handleClick(item)} key={item.id}>
                            <div>
